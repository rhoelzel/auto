...
! ---------- ------
  SUBROUTINE AUTOOP(AC,ICP,ICU)
    ...
    IF(IPS==15.AND.ABS(ISW)==1) THEN
       ...
       AP%NICP=NFPR-3
       IF(NFPRPREV<6)THEN
          CALL AUTOBV(AC,ICP,ICU,FNPO,BCPS,ICPO,STPNPO,FNCSBV)
       ELSE
          CALL AUTOBV(AC,ICP,ICU,FNPO,BCPS,ICPO,STPNBV,FNCSBV)
       ENDIF
    ELSE IF(IPS==5) THEN
       ...
       IF(NFPR==2) THEN
          AP%NDIM=NDIM+1
          IF(AP%IRS>0) THEN
             CALL AUTOAE(AC,ICP,ICU,FNC1,STPNAE,FNCSAE)
          ELSE
             CALL AUTOAE(AC,ICP,ICU,FNC1,STPNC1,FNCSAE)
          ENDIF
       ELSE
          AP%NDIM=2*NDIM+NFPR
          IF(MOD(ITP,10)/=2) THEN
             CALL AUTOAE(AC,ICP,ICU,FNC2,STPNAE,FNCSAE)
          ELSE
             CALL AUTOAE(AC,ICP,ICU,FNC2,STPNC2,FNCSAE)
          ENDIF
       ENDIF
    ENDIF
...
! ---------- ----
  SUBROUTINE FNC1(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
...
! ---------- ------
  SUBROUTINE STPNC1(AC,PAR,ICP,U,UDOT,NODIR)
...
! ---------- ----
  SUBROUTINE FNC2(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    ALLOCATE(DDP(NPAR))
    CALL FFC2(AC,NDIM,U,UOLD,ICP,PAR,F,NDM,DFDU,DFDP,DDP)
    ...
    DO I=1,NDIM
       UU=U(I)
       U(I)=UU-EP
       CALL FFC2(AC,NDIM,U,UOLD,ICP,PAR,FF1,NDM,DFU,DFP,DDP)
       U(I)=UU+EP
       CALL FFC2(AC,NDIM,U,UOLD,ICP,PAR,FF2,NDM,DFU,DFP,DDP)
       U(I)=UU
       DO J=1,NDIM
...
! ---------- ----
  SUBROUTINE FFC2(AC,NDIM,U,UOLD,ICP,PAR,F,NDM,DFDU,DFDP,DDP)

    TYPE(AUTOCONTEXT), INTENT(IN), TARGET :: AC
    TYPE(AUTOPARAMETERS), POINTER :: AP
    INTEGER, INTENT(IN) :: ICP(*),NDIM,NDM
    DOUBLE PRECISION, INTENT(IN) :: UOLD(NDIM)
    DOUBLE PRECISION, INTENT(INOUT) :: U(NDIM),PAR(*),DDP(AC%AP%NPAR)
    DOUBLE PRECISION, INTENT(OUT) :: F(NDIM)
...
! ---------- ------
  SUBROUTINE STPNC2(AC,PAR,ICP,U,UDOT,NODIR)
    ...
    IF(NFPR.EQ.3)THEN
       ...
       CALL NLVC(AC,NDM+1,NDM+1,1,DD,V)
       CALL NRMLZ(NDM+1,V)
       DO I=1,NDM+1
          U(NDM+I)=V(I)
       ENDDO
...
! ---------- ----
  SUBROUTINE FNPO(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FFPO(AC,U,UOLD,UPOLD,ICP,PAR,F,NDM,DFDU)
    ...
    DO I=1,NDIM
       UU=U(I)
       U(I)=UU-EP
       CALL FFPO(AC,U,UOLD,UPOLD,ICP,PAR,FF1,NDM,DFU)
       U(I)=UU+EP
       CALL FFPO(AC,U,UOLD,UPOLD,ICP,PAR,FF2,NDM,DFU)
       U(I)=UU
       DO J=1,NDIM
          DFDU(J,I)=(FF2(J)-FF1(J))/(2*EP)
       ENDDO
...
! ---------- ----
  SUBROUTINE FFPO(AC,U,UOLD,UPOLD,ICP,PAR,F,NDM,DFDU)
    ...
    CALL FUNI(AC,NDM,U,UOLD,ICP,PAR,1,F(NDM+1),DFDU,DUMDP)
...
! ---------- ----
  SUBROUTINE ICPO(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
    ...
    CALL FIPO(AC,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,NDM,DFU,DFP)
    ...
    DO I=1,NDIM
       UU=U(I)
       U(I)=UU-EP
       CALL FIPO(AC,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F1,NDM,DFU,DFP)
       U(I)=UU+EP
       CALL FIPO(AC,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F2,NDM,DFU,DFP)
       U(I)=UU
       DO J=1,NINT
...
! ---------- ----
  SUBROUTINE FIPO(AC,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,FI,NDM,DFDU,DFDP)
...
! ---------- ------
  SUBROUTINE STPNPO(AC,PAR,ICP,NTSR,NCOLRS,RLDOT,UPS,UDOTPS,TM,NODIR)
