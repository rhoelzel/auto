...
! ---------- ------
  SUBROUTINE AUTOBV(AC,ICP,ICU,FUNI,BCNI,ICNI,STPNBVI,FNCI)
...
!        are.
       ...
       RETURN
    ENDIF
    CALL CNRLBV(AC,ICP,ICU,FUNI,BCNI,ICNI,STPNBVI,FNCI)
...
!
!   this routine does not necessarily make sense
!   using AUTOCONTEXT instead of AUTOPARAMETERS - 
!   the AUTO adaptation for SCIGMA is not mean
!   to run with MPI
!
    integer iam,kwt
    include 'interfaces.h'
    ...
    call mpisbv(ap,par,icp,ndim,ups,uoldps,rlold,udotps,upoldp, &
         dtm,thu,ifst,nllv)
    dum=0
    call solvbv(ifst,ac,det,par,icp,funi,bcni,icni,dum, &
...
! ---------- ------
  SUBROUTINE CNRLBV(AC,ICP,ICU,FUNI,BCNI,ICNI,STPNBVI,FNCI)
...
! INITIALIZE COMPUTATION OF BRANCH
    ...
    NODIR=0
    CALL RSPTBV(AC,PAR,ICP,FUNI,STPNBVI,FNCI,RLCUR,RLOLD,RLDOT, &
...
!     don't set global rotations here for homoclinics, but in autlib5.f
    ...
    ENDIF
    IF(IPERP/=2)THEN
       CALL STDRBV(AC,PAR,ICP,FUNI,BCNI,ICNI,RLCUR,RLOLD,RLDOT, &
       ...
       IF(ISP/=0 .AND. (IPS==2.OR.IPS==7.OR.IPS==12) )THEN
          ...
          SP1 = FNCI(AC,ICP,UPS,NDIM,PAR,3,ATYPEDUM)
       ENDIF
    ENDIF
...
! Store plotting data for restart point :
    ...
    CALL STPLBV(AC,PAR,ICP,ICU,RLDOT,NDIM,UPS,UDOTPS,TM,DTM,THU,ISTOP)
    DO WHILE(ISTOP==0)
       ITP=0
       AP%ITP=ITP
       NINS=AP%NINS
       CALL STEPBV(AC,DSOLD,PAR,ICP,FUNI,BCNI,ICNI,FNCI,RDS, &
       ...
       ELSE
          ...
          DO ITEST=1,AP%NTEST
             ...
             CALL LCSPBV(AC,DSOLD,DSTEST,PAR,ICP,ITEST,FUNI,BCNI,ICNI,FNCI, &
       ...
       DO ITEST=1,ISTEPPED-1
          ...
          TEST(ITEST)=FNCS(AC,ICP,UPS,PAR,ATYPEDUM,IUZ,VUZ,ITEST,FNCI)
       ENDDO
...
! ---------- ------
  SUBROUTINE CONTBV(AC,DSOLD,PAR,ICP,FUNI,RLCUR,RLOLD,RLDOT, &
       NDIM,UPS,UOLDPS,UDOTPS,UPOLDP,DTM,THL,THU)
...
! The stepsize used in the preceding step has been stored in DSOLD.
    ...
    DOUBLE PRECISION, INTENT(IN) :: RLCUR(AC%AP%NFPR), DTM(*), THL(*), THU(*)
    DOUBLE PRECISION, INTENT(INOUT) :: RLOLD(AC%AP%NFPR)
    DOUBLE PRECISION, INTENT(OUT) :: RLDOT(AC%AP%NFPR)
...
! Rescale, to set the norm of (UDOTPS,RLDOT) equal to 1.
    CALL SCALEB(NTST,NCOL,NDIM,AC%AP%NFPR,UDOTPS,RLDOT,DTM,THL,THU)

! Store time-derivative.

    CALL STUPBV(AC,PAR,ICP,FUNI,NDIM,UPS,UPOLDP)
...
! ---------- ------
  SUBROUTINE STUPBV(AC,PAR,ICP,FUNI,NDIM,UPS,UPOLDP)
...
! Local
    ...
    DO J=0,NTST*NCOL
       UOLD(:)=UPS(:,J)
       CALL FUNI(AC,NDIM,UPS(1,J),UOLD,ICP,PAR,0,UPOLDP(1,J),&
            DFDU,DFDP)
    ENDDO
...
! ---------- ------
  SUBROUTINE STEPBV(AC,DSOLD,PAR,ICP,FUNI,BCNI,ICNI,FNCI,RDS, &
       RLCUR,RLOLD,RLDOT,NDIM,UPS,UOLDPS,UDOTPS,UPOLDP, &
       TM,DTM,P0,P1,THL,THU,NITPS,ISTOP)
...
! for the next solution (PAR(ICP(*)) , U) on a branch of solutions.
    ...
    DOUBLE PRECISION, INTENT(OUT) :: UPS(NDIM,0:*), RLCUR(AC%AP%NFPR)
    DOUBLE PRECISION, INTENT(IN) :: UOLDPS(NDIM,0:*), RLOLD(AC%AP%NFPR)
    DOUBLE PRECISION, INTENT(IN) :: UDOTPS(NDIM,0:*), RLDOT(AC%AP%NFPR), UPOLDP(NDIM,0:*)
...
! Perform Newton iterations
       ...
       CALL NEWTONBV(AC,PAR,ICP,FUNI,BCNI,ICNI,FNCI,RDS, &
            RLCUR,RLOLD,RLDOT,NDIM,UPS,UOLDPS,UDOTPS,UPOLDP, &
...
! ---------- --------
  SUBROUTINE NEWTONBV(AC,PAR,ICP,FUNI,BCNI,ICNI,FNCI,RDS, &
       RLCUR,RLOLD,RLDOT,NDIM,UPS,UOLDPS,UDOTPS,UPOLDP, &
       TM,DTM,P0,P1,THL,THU,NITPS,CONVERGED)
...
! Generate the Jacobian matrix and the right hand side.
    ...
    DO NIT1=1,ITNW
       ...
       CALL SOLVBV(IFST,AC,DET,PAR,ICP,FUNI,BCNI,ICNI,RDS,NLLV, &
...
! Check whether user-supplied error tolerances have been met :
       ...
       IF(DONE.AND.RDUMX.LT.EPSU)THEN
          IF(CHECKSP(AC,'LP',IPS,ILP,ISP))THEN
             ...
             CALL SOLVBV(IFST,AC,DET,PAR,ICP,FUNI,BCNI,ICNI,RDSZ,NLLV, &
...
! ---------- ------
  SUBROUTINE RSPTBV(AC,PAR,ICP,FUNI,STPNBVI,FNCI,RLCUR,RLOLD, &
...
! user-supplied subroutine STPNT.
    ...
    DOUBLE PRECISION RLCUR(AC%AP%NFPR),RLOLD(AC%AP%NFPR),RLDOT(AC%AP%NFPR)
...
! Get restart data :
    ...
    IF(NCOLRS*NTSRS==0)THEN
       ...
       CALL READLB(AC,ICPRS,U,UDOT,PAR)
...
! Generate the (initially uniform) mesh.
       ...
       IF(ITP==3 .OR. ABS(ITP/10)==3) THEN
          ...
          CALL PVLI(AC,ICP,UPS,NDIM,PAR,FNCI)
          ! Hopf bifurcation
          CALL STHOPF(AC,U,PAR,ICP,NTST,NCOL,NFPR,RLDOT, &
       ...
       DEALLOCATE(U,UDOT)
    ELSE
       CALL STPNBVI(AC,PAR,ICP,NTSRS,NCOLRS,RLDOT,UPS,UDOTPS,TM,NODIR)
    ENDIF
...
! ---------- ------
  SUBROUTINE STHOPF(AC,U,PAR,ICP,NTST,NCOL, &
...
!  bifurcation point (for waves or periodic orbits)
    ...
    DOUBLE PRECISION U(NDIM),PAR(*),RLDOT(AC%AP%NFPR),THU(*)
...
! Local
    ...
    CALL FUNI(AC,NDIM,U,U,ICP,PAR,1,F,DFU,DUMDFP)
...
! Note that the period-scaling in FUNC is taken into account:
    ...
    CALL NLVC(AC,2*NDIM,2*NDIM,2,SMAT,RNLLV)
...
! ---------- ------
  SUBROUTINE STDRBV(AC,PAR,ICP,FUNI,BCNI,ICNI,RLCUR,RLOLD, &
...
! the computation of a branch when no direction vector is given.
    ...
    DOUBLE PRECISION UDOTPS(NDIM,0:AC%AP%NTST*AC%AP%NCOL),DTM(*)
    DOUBLE PRECISION PAR(*),RLCUR(AC%AP%NFPR),RLOLD(AC%AP%NFPR),RLDOT(AC%AP%NFPR)
    DOUBLE PRECISION THL(*),THU(*),UPS(NDIM,0:AC%AP%NTST*AC%AP%NCOL)
...
! ---------- ------
  SUBROUTINE LCSPBV(AC,DSOLD,DSTEST,PAR,ICP,ITEST,FUNI,BCNI,ICNI,FNCI,Q, &
       RLCUR,RLOLD,RLDOT,NDIM,UPS,UOLDPS,UDOTPS,UPOLDP, &
...
! of branches of solutions to general boundary value problems.
    ...
    COMPLEX(KIND(1.0D0)), INTENT(INOUT) :: EV(AC%AP%NDM)
    DOUBLE PRECISION, INTENT(INOUT) :: PAR(*), UPS(NDIM,0:AC%AP%NCOL*AC%AP%NTST)
    DOUBLE PRECISION, INTENT(INOUT) :: UOLDPS(NDIM,0:AC%AP%NCOL*AC%AP%NTST)
    DOUBLE PRECISION, INTENT(INOUT) :: UDOTPS(NDIM,0:AC%AP%NCOL*AC%AP%NTST)
    DOUBLE PRECISION, INTENT(INOUT) :: UPOLDP(NDIM,0:AC%AP%NCOL*AC%AP%NTST)
    DOUBLE PRECISION, INTENT(IN) :: TM(*),DTM(*),THL(*),THU(*),VUZ(*)
    DOUBLE PRECISION, INTENT(INOUT) :: DSOLD,DSTEST,Q
    DOUBLE PRECISION, INTENT(INOUT) :: RLCUR(AC%AP%NFPR),RLOLD(AC%AP%NFPR)
    DOUBLE PRECISION, INTENT(INOUT) :: RLDOT(AC%AP%NFPR)
...
! Check for zero.
    ...
    Q0=Q
    Q1=FNCS(AC,ICP,UPS,PAR,ATYPE,IUZ,VUZ,ITEST,FNCI)
...
! Check for zero.
       ...
       Q=FNCS(AC,ICP,UPS,PAR,ATYPE,IUZ,VUZ,ITEST,FNCI)
...
! Return if tolerance has been met :
    ...
    NITPS=NITPSS
    CALL PVLI(AC,ICP,UPS,NDIM,PAR,FNCI)
    Q=FNCS(AC,ICP,UPS,PAR,ATYPEDUM,IUZ,VUZ,ITEST,FNCI)
    DEALLOCATE(UPSS,RLCURS,UOLDPSS,RLOLDS,UDOTPSS,RLDOTS,P0S,P1S)
...
! ---------- ------
  SUBROUTINE STPLBV(AC,PAR,ICP,ICU,RLDOT,NDIM,UPS,UDOTPS,TM,DTM,THU,ISTOP)
...
!  PAR(ICP(*)): Further free parameters (if any).
    ...
    DOUBLE PRECISION RLDOT(AC%AP%NFPR),UDOTPS(*)
! Local
...
! Determine stability, and write output on units 7 and 8.
    ...
    ENDIF
    CALL WRLINE(AC,PAR,ICU,IBRS,NTOTS,LABW,AMP,UMX)

! Write plotting and restart data on unit 8.

    IF(MOD(ITP,10).NE.0)THEN
       CALL WRTBV8(AC,PAR,ICP,RLDOT,NDIM,UPS,UDOTPS,TM,DTM)
       AP%LAB=AP%LAB+1
    ENDIF
...
! ---------- ------
  SUBROUTINE WRTBV8(AC,PAR,ICP,RLDOT,NDIM,UPS,UDOTPS,TM,DTM)
...
!  Above, RL-dot(.) and U-dot(.) specify the direction of the branch.
    ...
    DOUBLE PRECISION PAR(*),RLDOT(AC%AP%NFPR)
