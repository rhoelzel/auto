...
! Starting procedure  (to get direction vector) :
       ...
       IF(NODIR==1.AND.ISW>=0)THEN
          CALL STPRAE(AC,PAR,ICP,FUNI,U,UDOT,THU,0,AA)
       ELSEIF(IRS/=0.AND.ISW<0)THEN
          CALL STPRAE(AC,PAR,ICP,FUNI,U,UDOT,THU,1,AA)
...
! determine the second point on the bifurcating or original branch
          CALL STEPAE(AC,PAR,ICP,FUNI,RDS,AA,U,UDOT,THU,NIT,ISW<0)
...
! Find the next solution point on the branch
          ...
          ELSE
             ...
             DO ITEST=1,AP%NTEST
                ...
                CALL LCSPAE(AC,DSTEST,PAR,ICP,ITEST,FUNI,FNCI,AA,&
