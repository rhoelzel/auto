...
! ---------- ------
  SUBROUTINE AUTODS(AC,ICP,ICU)
    ...
    CASE(0)   ! ** Discrete dynamical systems : fixed points.
       CALL AUTOAE(AC,ICP,ICU,FNDS,STPNDS,FNCSDS)
    CASE(1)   ! BP continuation (maps)
       CALL AUTOAE(AC,ICP,ICU,FNBPDS,STPNBPDS,FNCSDS)
    CASE(2,7) ! ** PD or fold continuation (maps)
       CALL AUTOAE(AC,ICP,ICU,FNLPDS,STPNLPDS,FNCSDS)
    CASE(8)   ! Neimark-Sacker bifurcation continuation (maps)
       CALL AUTOAE(AC,ICP,ICU,FNNS,STPNNS,FNCSDS)
    END SELECT

  END SUBROUTINE AUTODS

! ---------- ----
  SUBROUTINE FNDS(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    IF(ITDS>=2)THEN
       ...
       DO I=2,ITDS
          ...
          FN(:)=F(:)
          CALL FUNI(AC,NDIM,FN,UOLD,ICP,PAR,IJAC,F,DFDU1,DFDP1)
          IF(IJAC>0)THEN
             ! DFDU=DFDU1*DFDU
...
! ---------- --------
  SUBROUTINE STPNDS(AC,PAR,ICP,U,UDOT,NODIR)
...
! ---------- ------
  SUBROUTINE FNLPDS(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FNLPF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FNDS)

  END SUBROUTINE FNLPDS

! ---------- --------
  SUBROUTINE STPNLPDS(AC,PAR,ICP,U,UDOT,NODIR)
    ...
    CALL STPNLPF(AC,PAR,ICP,U,UDOT,NODIR,FNDS)

  END SUBROUTINE STPNLPDS

! ---------- ------
  SUBROUTINE FNBPDS(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FNBPF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FNDS)

  END SUBROUTINE FNBPDS

! ---------- --------
  SUBROUTINE STPNBPDS(AC,PAR,ICP,U,UDOT,NODIR)
    ...
    CALL STPNBPF(AC,PAR,ICP,U,UDOT,NODIR,FNDS)

  END SUBROUTINE STPNBPDS

! ---------- ----
  SUBROUTINE FNNS(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FNHBF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FNDS,FFNSX)

  END SUBROUTINE FNNS

! ---------- -----
  SUBROUTINE FFNSX(AC,U,PAR,DFDU,DFDV)
...
! ---------- ------
  SUBROUTINE STPNNS(AC,PAR,ICP,U,UDOT,NODIR)
    ...
    CALL STPNHBF(AC,PAR,ICP,U,UDOT,NODIR,FNDS,FFNSX)

  END SUBROUTINE STPNNS

! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNCSDS(AC,ICP,U,NDIM,PAR,ITEST,ATYPE) RESULT(Q)
    ...
    SELECT CASE(ITEST)
    CASE(4)
       Q=FNRNDS(AC,ATYPE,U,AA)
    CASE(6)
       Q=FNHBDS(AC,PAR,ATYPE,AA)
    CASE DEFAULT
       Q=FNCSEQF(AC,ICP,U,NDIM,PAR,ITEST,ATYPE,FNDS)
    END SELECT

  END FUNCTION FNCSDS

! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNRNDS(AC,ATYPE,U,AA)
...
! Local
    ...
    IF(ITPST==2.OR.ITPST==7)THEN
       ! Rn on Fold/PD curve
       IF(AP%ITPST==2)THEN
          IF(.NOT.CHECKSP(AC,'R1',AP%IPS,AP%ILP,AP%ISP))RETURN
          ATYPE='R1'
       ELSE
          IF(.NOT.CHECKSP(AC,'R2',AP%IPS,AP%ILP,AP%ISP))RETURN
          ATYPE='R2'
       ENDIF
       FNRNDS=FNBTAE(AC,U,AA)
    ELSEIF(ITPST==8)THEN
       ...
       CASE DEFAULT ! 1:1 res
          ATYPE='R1'
       END SELECT
       IF(.NOT.CHECKSP(AC,ATYPE,AP%IPS,AP%ILP,AP%ISP))ATYPE=''
    ...
    IF(LEN_TRIM(ATYPE)>0.AND.AP%IID>=2)WRITE(AC%DUNIT,101)ABS(AP%IBR),NTOP+1,FNRNDS
101 FORMAT(I4,I6,9X,'Rn   Function:',ES14.5)

  END FUNCTION FNRNDS

! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNHBDS(AC,PAR,ATYPE,AA)
...
! Compute the eigenvalues of the Jacobian
    ...
    ELSE
       FNHBDS=REV
       IF(IID>=2)WRITE(AC%DUNIT,101)ABS(IBR),NTOP+1,FNHBDS
    ENDIF
    AP%HBFF=FNHBDS
