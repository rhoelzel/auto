...
! ---------- ------
  SUBROUTINE AUTOEQ(AC,ICP,ICU)
    ...
    CASE(0)
       ! Algebraic systems.
       CALL AUTOAE(AC,ICP,ICU,FUNI,STPNAE,FNCSEQ)
    CASE(1)
       ! ** BP cont (algebraic problems) (by F. Dercole).
       CALL AUTOAE(AC,ICP,ICU,FNBP,STPNBP,FNCSEQ)
    CASE(2)
       ! ** Fold continuation (algebraic problems).
       CALL AUTOAE(AC,ICP,ICU,FNLP,STPNLP,FNCSEQ)
    CASE(3)
       ! Hopf bifurcation continuation (ODEs).
       CALL AUTOAE(AC,ICP,ICU,FNHB,STPNHB,FNCSEQ)
    END SELECT
...
! ---------- ----
  SUBROUTINE FNHB(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FNHBF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FUNI,FFHBX)

  END SUBROUTINE FNHB

! ---------- -----
  SUBROUTINE FNHBF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FUNI,FUNIX)
    ...
    ELSE
       IJC=2
    ENDIF
    CALL FFHB(AC,NDIM,U,UOLD,ICP,PAR,IJC,F,NDM,DFU,DFP,FUNI,FUNIX)
    ...
    DO II=1,NDM+1
       ...
       UU=U(I)
       U(I)=UU-EP
       CALL FFHB(AC,NDIM,U,UOLD,ICP,PAR,0,FF1,NDM,DFU,DFP,FUNI,FUNIX)
       U(I)=UU+EP
       CALL FFHB(AC,NDIM,U,UOLD,ICP,PAR,0,FF2,NDM,DFU,DFP,FUNI,FUNIX)
       U(I)=UU
       DO J=NDM+1,NDIM
          IF(J/=NDIM-1)THEN
...
! ---------- ----
  SUBROUTINE FFHB(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,NDM,DFDU,DFDP,FUNI,FUNIX)
    ...
    CALL FUNIX(AC,U,PAR,DFDU,DFDU(1,NDM+1))
...
! ---------- -----
  SUBROUTINE FFHBX(AC,U,PAR,DFDU,DFDV)
...
! ---------- ------
  SUBROUTINE STPNHB(AC,PAR,ICP,U,UDOT,NODIR)
    ...
    CALL STPNHBF(AC,PAR,ICP,U,UDOT,NODIR,FUNI,FFHBX)

  END SUBROUTINE STPNHB

! ---------- -------
  SUBROUTINE STPNHBF(AC,PAR,ICP,U,UDOT,NODIR,FUNI,FUNIX)
    ...
    CALL FUNI(AC,NDM,U,UOLD,ICP,PAR,1,F,DFU,DFP)
...
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNCSEQ(AC,ICP,U,NDIM,PAR,ITEST,ATYPE) RESULT(Q)
    ...
    Q=FNCSEQF(AC,ICP,U,NDIM,PAR,ITEST,ATYPE,FUNI)

  END FUNCTION FNCSEQ

! ------ --------- -------- -------
  DOUBLE PRECISION FUNCTION FNCSEQF(AC,ICP,U,NDIM,PAR,ITEST,ATYPE,FUNI) &
       RESULT(Q)
    ...
    CASE(4) ! Check for Bogdanov-Takens bifurcation
       Q=FNBTEQ(AC,ATYPE,U,AA)
    CASE(5) ! Check for generalized Hopf (Bautin)
       Q=FNGHEQ(AC,PAR,ICP,ATYPE,FUNI,U,AA)
    CASE(6) ! Check for Hopf or Zero-Hopf
       Q=FNHBEQ(AC,PAR,ATYPE,AA)
    END SELECT

  END FUNCTION FNCSEQF

! ---------- --------
  SUBROUTINE PRINTEIG(AC)
    ...
    IF(AP%IID>0)THEN
       ...
       WRITE(AC%DUNIT,102)ABS(AP%IBR),NTOP+1,AP%NINS
       DO i=1,AP%NDM
          WRITE(AC%DUNIT,103)ABS(AP%IBR),NTOP+1,I,EVV(i)
       ENDDO
    ENDIF
...
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNHBEQ(AC,PAR,ATYPE,AA)
...
! Compute the eigenvalues of the Jacobian
    ...
    ENDIF
    IF(.NOT.CHECKSP(AC,ATYPE,AP%IPS,AP%ILP,ISP))THEN
       ATYPE=''
    ELSE
       FNHBEQ=REV
       IF(IID>=2)WRITE(AC%DUNIT,101)ABS(IBR),NTOP+1,FNHBEQ
    ENDIF
    AP%HBFF=FNHBEQ
...
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNBTEQ(AC,ATYPE,U,AA)
...
! Local
    ...
    FNBTEQ = 0
    ATYPE=''
    IF(.NOT.CHECKSP(AC,'BT',AP%IPS,AP%ILP,AP%ISP))THEN
       RETURN
    ENDIF
    ...
    NTOP=MOD(AP%NTOT-1,9999)+1
    IF(AP%IID.GE.2)WRITE(AC%DUNIT,101)ABS(AP%IBR),NTOP+1,FNBTEQ
101 FORMAT(I4,I6,9X,'BT   Function:',ES14.5)

  END FUNCTION FNBTEQ

! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNGHEQ(AC,PAR,ICP,ATYPE,FUNI,U,AA)
...
! Local
    ...
    FNGHEQ = 0
    ATYPE = ''
    IF(AP%ISW/=2.OR.AP%ITPST/=3.OR..NOT.CHECKSP(AC,'GH',AP%IPS,AP%ILP,AP%ISP))THEN
       RETURN
    ENDIF
    ...
    IF(AP%IID>=2)THEN
       NTOP=MOD(AP%NTOT-1,9999)+1
       WRITE(AC%DUNIT,"(I4,I6,9X,A,ES14.5)",ADVANCE="no")&
            ABS(AP%IBR),NTOP+1,'GH   Function:',FNGHEQ
       IF(FNGHEQ>0)THEN
          WRITE(AC%DUNIT,'(A)')' (subcritical)'
       ELSEIF(FNGHEQ<0)THEN
          WRITE(AC%DUNIT,'(A)')' (supercritical)'
       ELSE
          WRITE(AC%DUNIT,*)
       ENDIF
    ENDIF
    ...
    CONTAINS
      ...
      FUNCTION FN(x) RESULT(f)
        ...
        CALL FUNI(AC,n,x,x,ICP,PAR,0,f,dum,dum)
      END FUNCTION FN
