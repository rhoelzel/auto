...
! ---------- ------
  SUBROUTINE AUTOPS(AC,ICP,ICU)
    ...
    IF(ABS(ISW)<=1)THEN
       IF(IPS==2)THEN
          ! ** Periodic solutions
          CALL AUTOBV(AC,ICP,ICU,FNPS,BCPS,ICPS,STPNPS,FNCSPS)
       ELSE 
          ! Boundary value problems with Floquet multipliers. (IPS=7)
          CALL AUTOBV(AC,ICP,ICU,FUNI,BCNI,ICNI,STPNPS,FNCSPS)
       ENDIF
    ELSE ! here IPS=2
       IF(ABS(ISW)==2)THEN
          IF(ITP==5) THEN 
             ! ** Fold continuation (Periodic solutions, start).
             CALL AUTOBV(AC,ICP,ICU,FNPL,BCPL,ICPL,STPNPL,FNCSPS)
          ELSE IF((ABS(ITP)/10)==5)THEN
             ! ** Fold continuation (Periodic solutions, restart).
             CALL AUTOBV(AC,ICP,ICU,FNPL,BCPL,ICPL,STPNBV,FNCSPS)
          ELSE IF(ITP==7) THEN
             ! ** Continuation of period doubling bifurcations (start).
             CALL AUTOBV(AC,ICP,ICU,FNPD,BCPD,ICPD,STPNPD,FNCSPS)
          ELSE IF(ABS(ITP)/10==7)THEN
             ! ** Continuation of period doubling bifurcations (restart).
             CALL AUTOBV(AC,ICP,ICU,FNPD,BCPD,ICPD,STPNBV,FNCSPS)
          ELSE IF(ITP==8)THEN
             ! ** Continuation of torus bifurcations (start).
             CALL AUTOBV(AC,ICP,ICU,FNTR,BCTR,ICTR,STPNTR,FNCSPS)
          ELSE IF(ABS(ITP)/10==8)THEN
             ! ** Continuation of torus bifurcations (restart).
             CALL AUTOBV(AC,ICP,ICU,FNTR,BCTR,ICTR,STPNBV,FNCSPS)
          ENDIF
       ENDIF
       IF(ITP==6.OR.(ABS(ITP)/10)==6) THEN
          ! ** BP cont (Periodic sol., start and restart) (by F. Dercole).
          CALL AUTOBV(AC,ICP,ICU,FNPBP,BCPBP,ICPBP,STPNPBP,FNCSPS)
       ENDIF
    ENDIF
  END SUBROUTINE AUTOPS

! ---------- ----
  SUBROUTINE FNPS(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FUNI(AC,NDIM,U,UOLD,ICP,PAR,ABS(IJAC),F,DFDU,DFDP)
    PERIOD=PAR(11)
...
! ---------- ----
  SUBROUTINE ICPS(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
...
! ---------- ------
  SUBROUTINE STPNPS(AC,PAR,ICP,NTSR,NCOLRS,RLDOT,UPS,UDOTPS,TM,NODIR)
    ...
    CALL STPNBV1(AC,PAR,ICP,NDIM,NTSR,NDIMRD,NCOLRS, &
...
! ---------- ----
  SUBROUTINE FNPL(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FNPLF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FNPS)

  END SUBROUTINE FNPL

! ---------- -----
  SUBROUTINE FNPLF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FUNI)
    ...
    CALL FFPL(AC,U,UOLD,ICP,PAR,IJAC,F,NDM,DFDU,DFDP,FUNI)
    ...
    DO I=1,NDM
       UU=U(I)
       U(I)=UU-EP
       CALL FFPL(AC,U,UOLD,ICP,PAR,0,FF1,NDM,DFU,DFP,FUNI)
       U(I)=UU+EP
       CALL FFPL(AC,U,UOLD,ICP,PAR,0,FF2,NDM,DFU,DFP,FUNI)
       U(I)=UU
       DO J=NDM+1,NDIM
...
! ---------- ----
  SUBROUTINE FFPL(AC,U,UOLD,ICP,PAR,IJAC,F,NDM,DFDU,DFDP,FUNI)
    ...
    IF(IJC==0)IJC=-1
    IF(ICP(2)/=11)IJC=2
    CALL FUNI(AC,NDM,U,UOLD,ICP,PAR,IJC,F,DFDU,DFDP)
...
! ---------- ----
  SUBROUTINE BCPL(AC,NDIM,PAR,ICP,NBC,U0,U1,F,IJAC,DBC)
...
! ---------- ----
  SUBROUTINE ICPL(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
...
! ---------- ------
  SUBROUTINE STPNPL(AC,PAR,ICP,NTSR,NCOLRS,RLDOT,UPS,UDOTPS,TM,NODIR)
...
! ---------- -----
  SUBROUTINE FNPBP(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FFPBP(AC,NDIM,U,UOLD,UOLD(NDIM+NFPR+1),ICP,PAR,IJAC,F,NDM,DFDU,DFDP)
    ...
    DO I=1,NDM
       UU=U(I)
       U(I)=UU-EP
       CALL FFPBP(AC,NDIM,U,UOLD,UOLD(NDIM+NFPR+1),ICP,PAR,0,FF1,NDM,DFU,DFP)
       U(I)=UU+EP
       CALL FFPBP(AC,NDIM,U,UOLD,UOLD(NDIM+NFPR+1),ICP,PAR,0,FF2,NDM,DFU,DFP)
       U(I)=UU
       DO J=NDM+1,NDIM
          DFDU(J,I)=(FF2(J)-FF1(J))/(2*EP)
       ENDDO
...
! ---------- -----
  SUBROUTINE FFPBP(AC,NDIM,U,UOLD,UPOLD,ICP,PAR,IJAC,F,NDM,DFDU,DFDP)
    ...
    ELSE
       IJC=MAX(IJAC,1)
    ENDIF
    CALL FNPS(AC,NDM,U,UOLD,ICP,PAR,IJC,F,DFDU,DFDP)
...
! ---------- -----
  SUBROUTINE BCPBP(AC,NDIM,PAR,ICP,NBC,U0,U1,FB,IJAC,DBC)
...
! ---------- -----
  SUBROUTINE ICPBP(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,FI,IJAC,DINT)
    ...
    CALL FNPS(AC,NDM,U,UOLD,ICP,PAR,2,F,DFU,DFP)
...
! ---------- -------
  SUBROUTINE STPNPBP(AC,PAR,ICP,NTSR,NCOLRS,RLDOT,UPS,UDOTPS,TM,NODIR)
    ...
    DO J=0,NTSR*NCOLRS
       U(:)=UPST(:,J)
       CALL FNPS(AC2,NDM,U,U,ICPRS,PAR,0,UPOLDPT(1,J),DUM,DUM)
    ENDDO
    ...
    NLLV=1
    IFST=1
    CALL SOLVBV(IFST,AC2,DET,PAR,ICPRS,FNPS,BCPS,ICPS,RDSZ,NLLV, &
...
! ---------- ----
  SUBROUTINE FNPD(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FFPD(AC,U,UOLD,ICP,PAR,IJAC,F,NDM,DFDU,DFDP)
    ...
    DO I=1,NDM
       UU=U(I)
       U(I)=UU-EP
       CALL FFPD(AC,U,UOLD,ICP,PAR,0,FF1,NDM,DFU,DUMDP)
       U(I)=UU+EP
       CALL FFPD(AC,U,UOLD,ICP,PAR,0,FF2,NDM,DFU,DUMDP)
       U(I)=UU
       DO J=NDM+1,NDIM
          DFDU(J,I)=(FF2(J)-FF1(J))/(2*EP)
       ENDDO
...
! ---------- ----
  SUBROUTINE FFPD(AC,U,UOLD,ICP,PAR,IJAC,F,NDM,DFDU,DFDP)
    ...
    PERIOD=PAR(11)
    CALL FUNI(AC,NDM,U,UOLD,ICP,PAR,MAX(IJAC,1),F,DFDU,DFDP)
...
! ---------- ----
  SUBROUTINE ICPD(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
...
! ---------- ------
  SUBROUTINE STPNPD(AC,PAR,ICP,NTSR,NCOLRS,RLDOT,UPS,UDOTPS,TM,NODIR)
...
! ---------- ----
  SUBROUTINE FNTR(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FFTR(AC,U,UOLD,ICP,PAR,IJAC,F,NDM,DFDU,DFDP)
    ...
    DO I=1,NDM
       UU=U(I)
       U(I)=UU-EP
       CALL FFTR(AC,U,UOLD,ICP,PAR,0,FF1,NDM,DFU,DUMDP)
       U(I)=UU+EP
       CALL FFTR(AC,U,UOLD,ICP,PAR,0,FF2,NDM,DFU,DUMDP)
       U(I)=UU
       DO J=NDM+1,3*NDM
          DFDU(J,I)=(FF2(J)-FF1(J))/(2*EP)
       ENDDO
...
! ---------- ----
  SUBROUTINE FFTR(AC,U,UOLD,ICP,PAR,IJAC,F,NDM,DFDU,DFDP)
...
! ---------- ----
  SUBROUTINE ICTR(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
...
! ---------- ------
  SUBROUTINE STPNTR(AC,PAR,ICP,NTSR,NCOLRS,RLDOT,UPS,UDOTPS,TM,NODIR)
...
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNSPBV(AC,PAR,ATYPE,P0,P1,EV)
...
! of the unit circle or when a real eigenvalues passes through -1.
    ...
    DOUBLE PRECISION, INTENT(INOUT) :: PAR(*)
    CHARACTER(LEN=*), INTENT(OUT) :: ATYPE
    DOUBLE PRECISION, INTENT(IN) :: P0(AC%AP%NDIM,*),P1(AC%AP%NDIM,*)
...
! Initialize.
    ...
    IF(NDIM>NDM)THEN
       ...
       CALL NULLVC(AC,NDIM,NDM,0,Q1,U,IC,IRPIV)
       DEALLOCATE(Q1,IC,U)
       ...
       ENDIF
       IF(IID.GE.4)THEN
          CALL EVECS(AC,NDM,Q0,Q1,AP%ITPST==6)
       ENDIF
...
! Order the remaining Floquet multipliers by distance from |z|=1.
    ...
    IF(.NOT.CHECKSP(AC,ATYPE,AP%IPS,AP%ILP,ISP)) ATYPE=''
...
! ---------- -----
  SUBROUTINE EVECS(AC,NDIM,P0,P1,TRANS)
...
! Local
    ...
    CALL GEL(AC,NDIM,Q1,NDIM,P,Q0,DET)
    IF(TRANS)THEN
       JOBVL='V'
       JOBVR='N'
    ELSE
