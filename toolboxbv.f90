...
! ---------- -------
  SUBROUTINE AUTOBVP(AC,ICP,ICU)
    ...
    IF(ABS(ISW)<=1)THEN
       ...
       CALL AUTOBV(AC,ICP,ICU,FUNI,BCNI,ICNI,STPNBV,FNCSBV)
    ELSE
       ! Two-Parameter Continuation for IPS=4 or IPS=7.
       IF(ABS(ISW)==2)THEN
          IF(ITP==5)THEN
             ! ** Continuation of folds (BVP, start).
             CALL AUTOBV(AC,ICP,ICU,FNBL,BCBL,ICBL,STPNBL,FNCSBV)
          ELSE IF(ABS(ITP)/10==5) THEN
             ! ** Continuation of folds (BVP, restart).
             CALL AUTOBV(AC,ICP,ICU,FNBL,BCBL,ICBL,STPNBV,FNCSBV)
          ENDIF
       ENDIF
       IF((ITP==6.OR.(ABS(ITP)/10)==6)) THEN
          ! ** BP cont (BVP, start and restart) (by F. Dercole).
          CALL AUTOBV(AC,ICP,ICU,FNBBP,BCBBP,ICBBP,STPNBBP,FNCSBV)
       ENDIF
    ENDIF
...
! ---------- ------
  SUBROUTINE STPNBV(AC,PAR,ICP,NTSR,NCOLRS,RLDOT, &
       UPS,UDOTPS,TM,NODIR)
    ...
    DOUBLE PRECISION, INTENT(OUT) :: PAR(*),RLDOT(AC%AP%NFPR),TM(0:*)
    DOUBLE PRECISION, INTENT(OUT) :: UPS(AC%AP%NDIM,0:*),UDOTPS(AC%AP%NDIM,0:*)
    ...
    IF(AP%IRS==0)THEN
       CALL STPNUB(AC,PAR,RLDOT,UPS,UDOTPS,TM,NODIR)
       RETURN
    ENDIF

    ALLOCATE(UPSR(NDIM,0:NCOLRS*NTSR),UDOTPSR(NDIM,0:NCOLRS*NTSR), &
         TMR(0:NTSR))
    CALL STPNBV1(AC,PAR,ICP,NDIM,NTSRS,NDIMRD,NCOLRS,RLDOT, &
...
! ---------- -------
  SUBROUTINE STPNBV1(AC,PAR,ICP,NDIM,NTSRS,NDIMRD,NCOLRS,RLDOT, &
       UPS,UDOTPS,TM,NODIR)
...
! Local
    ...
    ALLOCATE(ICPRS(NFPR))
    ICPRS(:)=0
    CALL READBV(AC,PAR,ICPRS,NTSRS,NCOLRS,NDIMRD,RLDOT,UPS, &
...
! Generate the (initially uniform) mesh.
    ...
    IF(AC%DATFILE/='')THEN
       OPEN(AC%SUNIT,FILE=TRIM(AC%DATFILE),STATUS='old',ACCESS='sequential',&
            IOSTAT=ios)
       IF(ios/=0)THEN
          OPEN(AC%SUNIT,FILE=TRIM(AC%DATFILE)//'.dat',STATUS='old',&
       ...
       NTSR=-1
       DO
          READ(AC%SUNIT,*,END=2)TEMP,(TEMP,I=1,NDIM)
          NTSR=NTSR+1
       ENDDO
2      CONTINUE
...
! override parameter values with values from constants file
    ...
    DO I=1,SIZE(AC%UVALS)
       UPS(NAMEIDX(AC%UVALS(I)%INDEX,AC%unames),0:NTST*NCOL)=AC%UVALS(I)%VAR
    ENDDO
...
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNCSBV(AC,ICP,UPS,NDIM,PAR,ITEST,ATYPE) RESULT(Q)
    ...
    SELECT CASE(ITEST)
    CASE(0)
       CALL PVLSI(AC%AP,UPS,NDIM,PAR)
    CASE(1)
       Q=FNLPBV(AC,ATYPE)
    CASE(2)
       Q=FNBPBV(AC,ATYPE,AC%SUPPORT%P1V)
    END SELECT

  END FUNCTION FNCSBV
       
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNLPBV(AC,ATYPE)
...
! RETURNS A QUANTITY THAT CHANGES SIGN AT A LIMIT POINT (BVP)
    ...
    ATYPE=''
    FNLPBV=0d0
    IF(.NOT.CHECKSP(AC,'LP',AP%IPS,AP%ILP,AP%ISP))RETURN
    ...
    FNLPBV=AP%FLDF
    IF(IID.GE.2)THEN
       WRITE(AC%DUNIT,101)ABS(IBR),NTOP+1,FNLPBV
    ENDIF
...
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNBPBV(AC,ATYPE,P1)
...
! Local
    ...
    AP%BIFF=FNBPBV
    IF(.NOT.CHECKSP(AC,'BP',AP%IPS,AP%ILP,AP%ISP))RETURN
...
! Set the determinant of the normalized reduced system.
    ...
    IF(IID.GE.2)WRITE(AC%DUNIT,101)ABS(IBR),NTOP+1,FNBPBV
...
! ---------- ----
  SUBROUTINE FNBL(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FNBLF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FUNI)

  END SUBROUTINE FNBL

! ---------- -----
  SUBROUTINE FNBLF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FUNI)
    ...
    CALL FFBL(AC,U,UOLD,ICP,PAR,F,NDM,DFDU,DFDP,FUNI)
    ...
    DO I=1,NDM
       UU=U(I)
       U(I)=UU-EP
       CALL FFBL(AC,U,UOLD,ICP,PAR,FF1,NDM,DFU,DFP,FUNI)
       U(I)=UU+EP
       CALL FFBL(AC,U,UOLD,ICP,PAR,FF2,NDM,DFU,DFP,FUNI)
       U(I)=UU
       DO J=NDM+1,NDIM
...
! ---------- ----
  SUBROUTINE FFBL(AC,U,UOLD,ICP,PAR,F,NDM,DFDU,DFDP,FUNI)
    ...
    CALL FUNI(AC,NDM,U,UOLD,ICP,PAR,2,F,DFDU,DFDP)
...
! ---------- ----
  SUBROUTINE BCBL(AC,NDIM,PAR,ICP,NBC,U0,U1,F,IJAC,DBC)
    ...
    CALL BCBLF(AC,NDIM,PAR,ICP,NBC,U0,U1,F,IJAC,DBC,BCNI)

  END SUBROUTINE BCBL

! ---------- -----
  SUBROUTINE BCBLF(AC,NDIM,PAR,ICP,NBC,U0,U1,F,IJAC,DBC,BCNI)
    ...
    CALL FBBL(AC,NDIM,PAR,ICP,NBC0,U0,U1,F,DFU,BCNI)
    ...
    DO I=1,NDIM
       UU=U0(I)
       U0(I)=UU-EP
       CALL FBBL(AC,NDIM,PAR,ICP,NBC0,U0,U1,FF1,DFU,BCNI)
       U0(I)=UU+EP
       CALL FBBL(AC,NDIM,PAR,ICP,NBC0,U0,U1,FF2,DFU,BCNI)
       U0(I)=UU
       DO J=1,NBC
...
! ---------- ----
  SUBROUTINE FBBL(AC,NDIM,PAR,ICP,NBC0,U0,U1,F,DBC,BCNI)

    TYPE(AUTOCONTEXT), INTENT(INOUT), TARGET :: AC
    TYPE(AUTOPARAMETERS), POINTER :: AP
    INTEGER, INTENT(IN) :: NDIM,ICP(*),NBC0
    DOUBLE PRECISION, INTENT(INOUT) :: U0(NDIM),U1(NDIM),PAR(*)
    DOUBLE PRECISION, INTENT(OUT) :: F(NBC0+AC%AP%NDM+AC%AP%NFPR/2-1)
    ...
    NFPX=NFPR/2-1
    CALL BCNI(AC,NDM,PAR,ICP,NBC0,U0,U1,F,2,DBC)
    DO I=1,NBC0
       F(NBC0+I)=0.d0
       DO J=1,NDM
...
! ---------- ----
  SUBROUTINE ICBL(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
    ...
    CALL ICBLF(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT,ICNI)

  END SUBROUTINE ICBL

! ---------- -----
  SUBROUTINE ICBLF(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT,ICNI)
    ...
    CALL FIBL(AC,PAR,ICP,NINT,NNT0,U,UOLD,UDOT,UPOLD,F,DFU,ICNI)
    DO I=1,NFPX
    ...
    DO I=1,NDM
       UU=U(I)
       U(I)=UU-EP
       CALL FIBL(AC,PAR,ICP,NINT,NNT0,U,UOLD,UDOT,UPOLD,FF1,DFU,ICNI)
       U(I)=UU+EP
       CALL FIBL(AC,PAR,ICP,NINT,NNT0,U,UOLD,UDOT,UPOLD,FF2,DFU,ICNI)
       U(I)=UU
       DO J=NNT0+1,NINT-1
...
! ---------- ----
  SUBROUTINE FIBL(AC,PAR,ICP,NINT,NNT0,U,UOLD,UDOT,UPOLD,F,DINT,ICNI)
    ...
    CALL ICNI(AC,NDM,PAR,ICP,NNT0,U,UOLD,UDOT,UPOLD,F,2,DINT)
    DO I=1,NNT0
       F(NNT0+I)=0.d0
       DO J=1,NDM
...
! ---------- ------
  SUBROUTINE STPNBL(AC,PAR,ICP,NTSR,NCOLRS,RLDOT,UPS,UDOTPS,TM,NODIR)
    ...
    INTEGER, INTENT(OUT) :: NODIR
    DOUBLE PRECISION, INTENT(OUT) :: PAR(*),RLDOT(AC%AP%NFPR), &
         UPS(AC%AP%NDIM,0:*),UDOTPS(AC%AP%NDIM,0:*),TM(0:*)
    ! Local
    INTEGER, ALLOCATABLE :: ICPRS(:)
    DOUBLE PRECISION, ALLOCATABLE :: UPSR(:,:),UDOTPSR(:,:),TMR(:)
    ...
    CALL READBV(AC,PAR,ICPRS,NTSR,NCOLRS,NDIMRD,RLDOT,UPSR, &
...
! ---------- -----
  SUBROUTINE FNBBP(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FFBBP(AC,NDIM,U,UOLD,UOLD(NDIM+NFPR+1),ICP,PAR,F,NDM,DFDU,DFDP)
    ...
    DO I=1,NDIM
       UU=U(I)
       U(I)=UU-EP
       CALL FFBBP(AC,NDIM,U,UOLD,UOLD(NDIM+NFPR+1),ICP,PAR,FF1,NDM,DFU,DFP)
       U(I)=UU+EP
       CALL FFBBP(AC,NDIM,U,UOLD,UOLD(NDIM+NFPR+1),ICP,PAR,FF2,NDM,DFU,DFP)
       U(I)=UU
       DO J=1,NDIM
          DFDU(J,I)=(FF2(J)-FF1(J))/(2*EP)
       ENDDO
...
! ---------- -----
  SUBROUTINE FFBBP(AC,NDIM,U,UOLD,UPOLD,ICP,PAR,F,NDM,DFDU,DFDP)
    ...
    IF(NNT0>0) THEN
       ...
       CALL ICNI(AC,NDM,PAR,ICP,NNT0,U,UOLD,DUM,UPOLD,FI,1,DINT)
       DO I=1,NDM
          DO J=1,NNT0
...
! ---------- -----
  SUBROUTINE BCBBP(AC,NDIM,PAR,ICP,NBC,U0,U1,F,IJAC,DBC)
    ...
    ALLOCATE(DFU(NBC0,2*NDM+NPAR))
    CALL FBBBP(AC,NDIM,PAR,ICP,NBC,NBC0,U0,U1,F,DFU)
    ...
    DO I=1,NDIM
       UU=U0(I)
       U0(I)=UU-EP
       CALL FBBBP(AC,NDIM,PAR,ICP,NBC,NBC0,U0,U1,FF1,DFU)
       U0(I)=UU+EP
       CALL FBBBP(AC,NDIM,PAR,ICP,NBC,NBC0,U0,U1,FF2,DFU)
       U0(I)=UU
       DO J=1,NBC
          DBC(J,I)=(FF2(J)-FF1(J))/(2*EP)
       ENDDO
  ...
  !     ---------- -----
  SUBROUTINE FBBBP(AC,NDIM,PAR,ICP,NBC,NBC0,U0,U1,FB,DBC)
    ...
    CALL BCNI(AC,NDM,PAR,ICP,NBC0,U0,U1,FB,2,DBC)
...
! ---------- -----
  SUBROUTINE ICBBP(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
    ...
    ELSE
       ALLOCATE(DFU(1))
    ENDIF
    CALL FIBBP(AC,NDIM,PAR,ICP,NINT,NNT0,U,UOLD,UDOT,UPOLD,F,DFU)
    ...
    DO I=1,NDIM
       UU=U(I)
       U(I)=UU-EP
       CALL FIBBP(AC,NDIM,PAR,ICP,NINT,NNT0,U,UOLD,UDOT,UPOLD,FF1,DFU)
       U(I)=UU+EP
       CALL FIBBP(AC,NDIM,PAR,ICP,NINT,NNT0,U,UOLD,UDOT,UPOLD,FF2,DFU)
       U(I)=UU
       DO J=1,NINT
          DINT(J,I)=(FF2(J)-FF1(J))/(2*EP)
       ENDDO
  ...
  !     ---------- -----
  SUBROUTINE FIBBP(AC,NDIM,PAR,ICP,NINT,NNT0,U,UOLD,UDOT,UPOLD,FI,DINT)
    ...
    CALL FUNI(AC,NDM,U,UOLD,ICP,PAR,2,F,DFU,DFP)
    IF(NNT0>0) THEN
       CALL ICNI(AC,NDM,PAR,ICP,NNT0,U,UOLD,UDOT,UPOLD,FI,2,DINT)
...
! ---------- -------
  SUBROUTINE STPNBBP(AC,PAR,ICP,NTSR,NCOLRS,RLDOT,UPS,UDOTPS,TM,NODIR)
    ...
    IF(ISW>0) THEN
       !        ** restart
       CALL STPNBV(AC,PAR,ICP,NTSR,NCOLRS,RLDOT,UPS,UDOTPS,TM,NODIR)
       RETURN
    ENDIF
    ...
    ! ** read the std branch
    CALL READBV(AC,PAR,ICPRS,NTSR,NCOLRS,NDIMRD,RLDOTRS,UPST, &
    ...
    IF(NNT0>0) THEN
       DO J=0,NTSR*NCOLRS
          U(:)=UPST(:,J)
          CALL FUNI(AC2,NDM,U,U,ICPRS,PAR,0,UPOLDPT(1,J),DUM,DUM)
       ENDDO
    ENDIF
    ...
    NLLV=1
    IFST=1
    CALL SOLVBV(IFST,AC2,DET,PAR,ICPRS,FUNI,BCNI,ICNI,RDSZ,NLLV, &
    ...
    CALL ADAPT2(NTSR,NCOLRS,NDIM,AC%AP%NTST,AC%AP%NCOL,NDIM, &
         TMR,UPSR,UDOTPSR,TM,UPS,UDOTPS,.FALSE.)
    DEALLOCATE(TMR,UPSR,UDOTPSR)
