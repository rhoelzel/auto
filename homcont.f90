  ...
  SUBROUTINE AUTOHO(AC,ICP,ICU)
    ...
    IF(ABS(AP%ISW)==2.AND.(AP%ITP==5.OR.(ABS(AP%ITP)/10)==5))THEN
       IF(AP%ITP==5) THEN 
          ...
          CALL AUTOBV(AC,ICP,ICU,FNHOL,BCHOL,ICHOL,STPNHO,FNCSHO)
       ELSE
          ! ** Fold continuation (restart).
          CALL AUTOBV(AC,ICP,ICU,FNHOL,BCHOL,ICHOL,STPNHO,FNCSHO)
       ENDIF
    ELSE
       CALL AUTOBV(AC,ICP,ICU,FNHO,BCHO,ICHO,STPNHO,FNCSHO)
    ENDIF

  END SUBROUTINE AUTOHO

! ---------- ----
  SUBROUTINE FNHO(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    IF(NDIM==NDM)THEN
       ...
       CALL FFHO(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,11)
       RETURN
    ENDIF
    ...
    IF(AP%NPARI>0)THEN
       ...
       DO J=0,NDIM-NDM,NDM
          ...
          ELSE
             IT=19+(J/NDM)*2
          ENDIF
          CALL FFHO(AC,NDM,U(J+1),UOLD(J+1),ICP,PAR,IJAC,F(J+1),DFU,DFP,IT)
          IF(IJAC/=0)THEN
    ...
    IF(IJAC/=0)THEN
       ...
       DO I=1,NDM
          UU=U(I)
         U(I)=UU-EP
          CALL FFHO(AC,NDIM,U,UOLD,ICP,PAR,1,DFDU(1,I),DFU,DFP,11)
          U(I)=UU+EP
          CALL FFHO(AC,NDIM,U,UOLD,ICP,PAR,1,F,DFU,DFP,11)
          U(I)=UU
          DO J=NDM+1,NDIM
    ...
    CALL FFHO(AC,NDIM,U,UOLD,ICP,PAR,MAX(IJAC,1),F,DFU,DFP,11)
    IF(IJAC==0)THEN
       DEALLOCATE(DFU,DFP)
       RETURN
    ENDIF
    ...
    DO I=1,NFPR
       ...
       EP=HMACH*( 1 +ABS(P) )
       PAR(ICP(I))=P+EP
       CALL FFHO(AC,NDIM,U,UOLD,ICP,PAR,1,FF1,DFU,DFP,11)
       DO J=NDM+1,NDIM
...
! ---------- ----
  SUBROUTINE FFHO(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,IT)
    ...
    DOUBLE PRECISION, INTENT(INOUT) :: U(NDIM),PAR(*)
    DOUBLE PRECISION, INTENT(OUT) :: F(NDIM)
    DOUBLE PRECISION, INTENT(INOUT) :: DFDU(AC%AP%NDM,AC%AP%NDM)
    DOUBLE PRECISION, INTENT(INOUT) :: DFDP(AC%AP%NDM,*)
    ...
    CALL FUNI(AC,NDM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
...
! ---------- ----
  SUBROUTINE BCHO(AC,NDIM,PAR,ICP,NBC,U0,U1,F,IJAC,DBC)
    ...
    CALL FBHO(AC,NDIM,PAR,ICP,NBC,.TRUE.,U0,U1,F)
    ...
    DO I=1,NDIM
       UU(I)=U0(I)-EP
       CALL FBHO(AC,NDIM,PAR,ICP,NBC,.FALSE.,UU,U1,FF1)
       UU(I)=U0(I)+EP
       CALL FBHO(AC,NDIM,PAR,ICP,NBC,.FALSE.,UU,U1,FF2)
       UU(I)=U0(I)
       DO J=1,NBC
    ...
    DO I=1,NDIM
       UU(I)=U1(I)-EP
       CALL FBHO(AC,NDIM,PAR,ICP,NBC,.FALSE.,U0,UU,FF1)
       UU(I)=U1(I)+EP
       CALL FBHO(AC,NDIM,PAR,ICP,NBC,.FALSE.,U0,UU,FF2)
       UU(I)=U1(I)
       DO J=1,NBC
    ...
    DO I=1,NFPR
       P=PAR(ICP(I))
       PAR(ICP(I))=P+EP
       CALL FBHO(AC,NDIM,PAR,ICP,NBC,.FALSE.,U0,U1,FF2)
       DO J=1,NBC
...
! ---------- ----
  SUBROUTINE ICHO(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
    ...
    IF (JB<NINT) THEN
       IF(IJAC/=0)THEN
          ...
          CALL ICNI(AC,NDM,PAR,ICP,NINT-JB,U,UOLD,UDOT,UPOLD,F(JB),IJAC,DNT)
          ...
          DEALLOCATE(DNT)
       ELSE
          CALL ICNI(AC,NDM,PAR,ICP,NINT-JB,U,UOLD,UDOT,UPOLD,F(JB),0,DUM1)
       ENDIF
    END IF
...
! ---------- -----
  SUBROUTINE FNHOL(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FNBLF(AC,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FNHO)

  END SUBROUTINE FNHOL

! ---------- ----
  SUBROUTINE BCHOL(AC,NDIM,PAR,ICP,NBC,U0,U1,F,IJAC,DBC)
    ...
    CALL BCBLF(AC,NDIM,PAR,ICP,NBC,U0,U1,F,IJAC,DBC,BCHO)

  END SUBROUTINE BCHOL

! ---------- -----
  SUBROUTINE ICHOL(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
    ...
    CALL ICBLF(AC,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT,ICHO)
  ...
  SUBROUTINE INHO(AC,ICP)
    ...
    ELSE
       ...
       IF (AC%HOMCONT%IEQUIB==2.AND.AP%IID>0) THEN 
          WRITE(AC%DUNIT,*)'WARNING: IEQUIB=2 NOT ALLOWED WITH ISTART=3'
       ENDIF
       IF (AC%HOMCONT%IEQUIB<0) THEN
...
! ---------- ------
  SUBROUTINE STPNHO(AC,PAR,ICP,NTSR,NCOLRS,RLDOT,UPS,UDOTPS,TM,NODIR)
    ...
    DO J=0,NTST*NCOL
       ...
       ENDDO
       IF(AP%IID>0)THEN
          write(AC%DUNIT,111)(ups(k,j),k=1,ndim)
       ENDIF
...
! ------ --------- --------
  DOUBLE PRECISION FUNCTION FNCSHO(AC,ICP,UPS,NDIM,PAR,ITEST,ATYPE)
    ...
    FNCSHO=FNCSBV(AC,ICP,UPS,NDIM,PAR,ITEST,ATYPE)
    IF(ITEST>0)RETURN
    ...
    ENDIF
    IF(IID.GE.3)THEN
       WRITE(AC%DUNIT,*) 'EIGENVALUES'
       DO J=1,NDM
          WRITE(AC%DUNIT,101) RR(J,1),RI(J,1)
       ENDDO
       IF(AC%HOMCONT%IEQUIB<0)THEN
          WRITE(AC%DUNIT,*) 'EIGENVALUES of RHS equilibrium'
          DO J=1,NDM
             WRITE(AC%DUNIT,101) RR(J,2),RI(J,2)
          ENDDO
       ENDIF
    ENDIF
    ...
    IF ((AC%HOMCONT%ITWIST==1).AND.(AC%HOMCONT%ISTART.GE.0)) THEN
       ...
       IF(IID.GE.3)THEN
          IF (ORIENT<0.0D0) THEN
             WRITE(AC%DUNIT,102) ORIENT             
          ELSE
             WRITE(AC%DUNIT,103) ORIENT   
          ENDIF
       ENDIF
    ENDIF
    ...
    DO I=1,AC%HOMCONT%NPSI
       ...
       IF(IID.GE.3)WRITE(AC%DUNIT,104)AC%HOMCONT%IPSI(I),PAR(20+AC%HOMCONT%IPSI(I))
    ENDDO
...
! ---------- -----
  SUBROUTINE EIGHO(AC,ITRANS,RR,RI,VRET,XEQUIB,ICP,PAR,NDM)
    ...
    CALL FUNI(AC,NDM,XEQUIB,XEQUIB,ICP,PAR,1,F,DFDU,DFDP)
    IF(PAR(11)<0)THEN
    ...
    IF (IFAIL/=0.AND.AC%AP%IID>0) THEN   
       WRITE(AC%DUNIT,*) 'LAPACK EIGENVALUE ROUTINE FAILED !'
    ENDIF
...
! ---------- ------
  SUBROUTINE PRJCTN(AC,BOUND,CSAVE,XEQUIB,ICP,PAR,IMFD,IS,ITRANS,NDM)
    ...
    CALL FUNI(AC,NDM,XEQUIB,XEQUIB,ICP,PAR,1,FDUM,A,DDUM)
    IF(PAR(11)<0)THEN
       ! reverse time: reverse eigenvalues/vectors
       A(:,:)=-A(:,:)
    ENDIF
    ...
    IF (IFAIL==0.AND.ISDIM/=MCOND) THEN
       ...
       IF (IFAIL/=0.AND.AC%AP%IID>0) THEN
          WRITE(AC%DUNIT,*)'LAPACK SCHUR DECOMPOSITION ROUTINE FAILED !'
       ENDIF
    ENDIF
    ...
    ! D = DUM1^-1 * DUM2
    CALL GEL(AC,MCOND,DUM1,MCOND,D,DUM2,DET)
